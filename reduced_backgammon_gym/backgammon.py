...
class Backgammon:
    ...
    def new_is_valid(self, color, action):
        ...
        if color == self.white:
            ...
            if not self.is_src_furthest_piece(color, action) and src - abs(roll) < -1:
                return False
        ...
        for action in temp:
            ...
            if color == self.white:
                ...
                if src == 7:
                    ...
                    final.append((abs(-1 - src), (action)))
                else:
                    final.append((abs(dst - src), (action)))
...
if __name__ == '__main__':
    ...
    bg = Backgammon()
    print(bg.get_valid_actions(1))
    print(bg.alternate_generate_actions(1))
    print(set(bg.get_valid_actions(1)).issubset(bg.alternate_generate_actions(1)))
